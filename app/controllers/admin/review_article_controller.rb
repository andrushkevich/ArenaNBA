class Admin::ReviewArticleController < AdminController
  ...
  def get_players
    if params["article_players_real_rate_article_id"]
      en_article, ru_article, guest_players, home_players, players_rate = get_params_for_form(params["article_players_real_rate_article_id"].to_i)
      render :update do |page|
  ...
  def set_object_type
    @object_name = "ArticlePlayersRealRate"
  end
  
  def get_class
    ArticlePlayersRealRate
  end
  ...
  def bind_players
    players = []
    game = Article.find(params["article_players_real_rate"]["article_id"].to_i).game
    if game      
      params["team_guest"].each_pair { |key, value| 
        if !value["score"].blank? || !value["minute"].blank?
          ...
          item.real_rate = value["score"].to_i
          item.real_time = value["minute"].to_i
          item.winner_type = winner_type
          players << item
        else
          item = PlayerGame.find_by_player_id_and_game_id(key.to_i, game.id)
          if item
            if item.matchup_rate || item.matchup_time
              item.real_rate = item.real_time = nil
            else
              item.destroy
            end          
          end
  ...
  def fill_article(item,language)
    item.article_id = params["article_players_real_rate"]["article_id"]
